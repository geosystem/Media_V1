                ...
                {                           
                    ...
                    case 'statusupload' :
                        ...
                        if($lKonfig['kon_upload'] == 0){
                            $uStat = "NON-AKTIF"; $uVal=1;$iIcn='fa-lock';$uStatus="AKTIF";$x='tidak';
                            $btn = "Aktifkan";
                        } else {
                            $uStat = "AKTIF"; $uVal=0;$iIcn='fa-unlock';$uStatus="NON-AKTIF";$x='';
                            $btn = "Non-Aktifkan";
                        }
                        if($act=='modalubahstatus'){
                            ...
                            <div class='modal inmodal' id='config' tabindex='-1' role='dialog' aria-hidden='true' data-keyboard='false' data-backdrop='static'>
                                <div class='modal-dialog'>
                                    <div class='modal-content animated fadeIn'>
                                        ...
                                        <div class='modal-body'>
                                            <h3 class='text-center'><strong class='text-danger'><i class='fa $iIcn'></i> STATUS UPLOAD $uStat</strong></h3>
                                            <p>User $x bisa melakukan upload video, apakah anda akan mengubah status upload video menjadi <strong>$uStatus</strong>?</p>                                         
                                        </div>
                                        <div class='modal-footer'>
                                            <a href='./' class='btn btn-white'>Batal</a>
                                            <a href='?page=".base64_encode('statusupload')."&act=".base64_encode('execubahstatus')."&plcode=".base64_encode($plcode)."&status=".base64_encode($uVal)."' class='btn btn-danger btn-outline'>$btn status upload</a>
                                        </div>
                            ...
                            $eval = base64_decode($_GET['status']);
                            if($eval == 1) {
                                $eket = "status upload video berhasil diubah menjadi <strong>AKTIF</strong>";$cval=1;
                            } else {
                                $eket = "status upload video berhasil diubah menjadi <strong>NON-AKTIF</strong>";$cval=0;
                            }
                            //update stust uploa
                            $sUpdv = mysqli_query($ppdb,"UPDATE reg_konfig SET kon_upload = '$cval'");
                            echo "
                            <script>
                                setTimeout(function() {
                                    swal({
